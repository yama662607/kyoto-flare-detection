...
本プロジェクトをすぐに開始するための手順です。

### 1. ツール一式の準備
まずは環境を整えるために `uv` と `just` をインストールしてください（詳細は[セットアップガイド](#-セットアップガイド-setup-guide)を参照）。
...
### 3. 解析の準備と開始
1. **データの配置**:
   TESS の FITS データを `data/TESS/<star_name>/` に配置してください。
2. **ノートブックの実行**:
   `notebooks/` ディレクトリ内の Jupyter Notebook (`.ipynb`) を開いて解析を開始してください。
