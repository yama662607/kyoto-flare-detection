...
class FlareDetector:
    ...
    def __init__(
        ...
        self.rot_period = 0.3672257916463397
    ...
    def load_TESS_data(self):
        ...
        self.flux_mean = 119633.99533564149
    ...
    def reestimate_errors(self):
        ...
        print(np.mean(err))
        err_constant = np.mean([0.0005519882005171186,
    ...
    def tess_band_energy(self, count):
        ...
        star_intensity = np.sum(dw * self.planck(wave * 1e-9, self.T_star) * resp)* Rstar**2 + np.sum(dw * self.planck(wave * 1e-9, 4700) * resp)* (0.864*Rsun_cm)**2
    ...
    def flux_diff(self, min_flux=0.02, max_flux=0.98): ##! このメゾットのみを呼び出しても、現状ではエネルギー情報は更新されない
        ...
        self.starspot = 2 * np.pi * ( (self.R_sunstar_ratio*695510e3)**2 + ( 0.864*695510e3)**2 ) * (self.T_star**4 / (self.T_star**4 - (self.T_star - self.d_T_star)**4)) *self.brightness_variation_amplitude
    ...
    def plt_flare(self):
        ...
        if self.s2mPDCSAPflux is not None:
            ...
            if self.peaktime is not None:
                ymin=np.max(self.s2mPDCSAPflux*1.1)
                ymax=np.max(self.s2mPDCSAPflux*1.3)
        ...
        output_dir = MODULE_DIR / 'DS Tuc A'
        output_dir.mkdir(parents=True, exist_ok=True)
        plt.savefig(output_dir / 's0001_DSTucA_lightcurve.pdf', format='pdf', bbox_inches='tight')
    ...
    def plot_flare(self):
        ...
        if self.s2mPDCSAPflux is not None:
            ...
            fig.update_xaxes(title_text="Time (min) (BJD - 2457000)", row=2, col=1)
